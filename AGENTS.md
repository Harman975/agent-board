...
```
  ...
  ├── decomposer.ts    Smart task decomposer — import graph, coupling clusters, Codex CLI
...
AgentBoard exposes typed MCP tools for any MCP-compatible client (Codex, Cursor, Windsurf, etc.).
...
```
board sprint suggest <goal>         # Smart decomposition — analyzes imports, coupling, calls Codex
board sprint suggest <goal> --auto  # Auto-decompose (calls Codex CLI)
